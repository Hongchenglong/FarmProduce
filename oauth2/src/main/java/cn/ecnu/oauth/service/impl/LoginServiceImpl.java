...
public class LoginServiceImpl implements LoginService {
    ...
    public AuthToken login(String username, String password, String clientId, String clientSecret, String grandType) {
        ...
        try {
            ...
            ServiceInstance choose = loadBalancerClient.choose("oauth");
    ...
    //短信验证登录
    @Override
    public AuthToken vlogin(String telephone, String validateCode, String clientId, String clientSecret, String grandType){
        try {
            //1.定义url (申请令牌的url)
            //参数 : 微服务的名称spring.appplication指定的名称
            ServiceInstance choose = loadBalancerClient.choose("oauth");
            ...
            //3. 定义请求体  有授权模式 用户的名称 和密码
            MultiValueMap<String,String> formData = new LinkedMultiValueMap<>();
            formData.add("grant_type",grandType);//自定义授权类型
            formData.add("telephone",telephone);
            formData.add("smscode",validateCode);//授权码
            //4.模拟浏览器 发送POST 请求 携带 头 和请求体 到认证服务器
