...
public class DateUtil {
    ...
    public static final String yyyy_MM_dd_HH = "yyyy年MM月dd日HH时";
    /** 时间格式HHmmss **/
     ...
     * @return
     */
    public static String ldt2Four(LocalDateTime localDateTime){
    ...
    public static LocalDateTime strToLdt(String timeStr){
        ...
        DateTimeFormatter df = DateTimeFormatter.ofPattern(FORMAT_TIME_SHOW);
    ...
    /**
     * 返回默认时间格式 15位:yyMMddHHmmssSSS
     *
     * @author lfjiang 2017年8月18日
     * @return
     */
    public static String getCurrYYMMddHHmmssSSS() {
        return getCurrentDate(yyMMddHHmmssSSS);
    }
    ...
    public static String addYears(String format,String date, int years) {
        ...
        Date d = parseDate(format, date);
        Calendar cal = Calendar.getInstance();
        cal.setTime(d);
        cal.add(Calendar.YEAR, years);
        return parseDate(format, cal.getTime());
    }

    /**
     * 为日期增加秒数
     *
     * @author lfjiang 2017年2月25日
     * @param curDateType
     * @param seconds
     * @return
     */
    ...
    /**
     * 判断目标日期是否在起始日期和结束日期之间，闭区间返回true
     *
     * @author lfjiang 2019年11月5日
     * @param format
     * @param startTime
     * @param endTime
     * @param targetTime
     * @return
     */
    public static boolean isInRegion(String format, String startTime, String endTime, String targetTime) {
        if (StringUtils.isBlank(format) || StringUtils.isBlank(targetTime)) {
            return false;
        }
        Date target = parseDate(format, targetTime);
        if (StringUtils.isNotBlank(startTime)) {
            Date start = parseDate(format, startTime);
            if (start.after(target)) {
                return false;
            }
        }
        if (StringUtils.isNotBlank(endTime)) {
            Date end = parseDate(format, endTime);
...
//            String yyyyMMddHH = data2str(dateMenu, "yyyyMMddHH");
