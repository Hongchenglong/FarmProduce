...
public class ServiceExceptionUtil {
    ...
    public static ServiceException exception(ErrorCode errorCode) {
        String shopLocale = ShopContextHolder.getShopLocale();
        LOGGER.info("shopLocale:" + shopLocale);
        if (!StringUtils.isEmpty(shopLocale))
            return exception(shopLocale, errorCode);
        else {
    ...
    public static ServiceException exception(ErrorCode errorCode, Object... params) {
        String shopLocale = ShopContextHolder.getShopLocale();
        if (!StringUtils.isEmpty(shopLocale))
            return exception(shopLocale, errorCode, params);
        else {
            String language = CommonConstants.DEFAULT_LANGUAGE;
            Map<String, String> map = messages.get(errorCode.getCode());
            if (map != null) {
