package cn.ecnu.system.mapper;

import cn.ecnu.system.pojo.Enviroment;
import com.baomidou.mybatisplus.core.mapper.BaseMapper;
import org.apache.ibatis.annotations.Delete;
import org.apache.ibatis.annotations.Select;

import java.util.List;

/****
 * @Author: Kili
 * @Description: EnviromentDao
 * @Date 2022-04-18 21:25:40
 *****/
public interface EnviromentMapper extends BaseMapper<Enviroment> {
    /**
     * 根据id查询已经逻辑删除的数据
     */
    @Select("select * from enviroment where id = #{id} and deleted=1")
    public Enviroment findLogicDelById(Long id);

    /**
     * 查询所有已经逻辑删除的数据
     */
    @Select("select * from enviroment where deleted=1")
    public List<Enviroment> findAllLogicDel();

    /**
     * 根据id物理删除数据
     */
    @Delete("delete from enviroment where id=#{id}")
    public Boolean deleteTablelogic(Long id);

    /**
     * 根据id更新逻辑删除的数据
     * 复杂sql语句写在xml里面
     * @param enviroment
     */
    public Boolean updateLogicDelById(Enviroment enviroment);

}
