...
public class EnvironmentController {
    ...
    public R delete(@PathVariable  Long id){
        ...
        return R.ok(environmentService.removeById(id));
    }
    ...
    @PostMapping
    public R add(@RequestBody  @ApiParam(name = "Environment对象",value = "传入JSON数据",required = true) Environment environment){
        //调用EnvironmentService实现添加Environment
        return R.ok(environmentService.save(environment));
    }
    ...
    @GetMapping
    public R<List<Environment>> findAll(){
        //调用EnvironmentService实现查询所有Environment
        return R.ok(environmentService.list(Wrappers.<Environment>lambdaQuery().orderByDesc(Environment::getCreateTime))) ;
    }
}
